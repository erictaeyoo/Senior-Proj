...
class DeepfakeDetector(nn.Module):
    def __init__(self, config):
        ...
        self.classifier_head = nn.Sequential(
            nn.LayerNorm(self.feature_dim_rgb + self.feature_dim_edge),
            nn.Linear(self.feature_dim_rgb + self.feature_dim_edge, 256),  # Increased FC layer size
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
